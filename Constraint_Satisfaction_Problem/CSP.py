...
def performCSP(NewScheduleArray,check):
    global checkFlag
    flag = False
    if checkSuccess():
        return "Success"
            ...
            for sel in NewScheduleArray:
                if sel.className != unassignedClass.className:
                    ...
                    if sum == 0 and not checkSuccess():
                        ...
                        keyword = performCSP(NewScheduleArray,check)
...
def inconsistent(arc):
    flagInconsistent = False
    for x in arc.domainTA:
        for y in arc.neigbhor:
            for z in y.domainTA:
                if x != z:
                    flagInconsistent = True
    
    if not flagInconsistent:
        print "Remove",x.TAName, "from",arc.className
        arc.domainTA.remove(x)
    return not flagInconsistent



def CSP(NewScheduleArray,check):
    q = Queue.Queue()
    for sel in NewScheduleArray:
        for x in xrange(1,len(NewScheduleArray)):
            if not set(sel.domainTA).isdisjoint(NewScheduleArray[x].domainTA):
                #print sel.className,NewScheduleArray[x].className
                sel.neigbhor.append(NewScheduleArray[x])
        q.put(sel)
    while not q.empty():
        arcs = q.get()
        if inconsistent(arcs):
            q.put(arcs)
    performCSP(NewScheduleArray,check)
