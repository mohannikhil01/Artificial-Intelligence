...
def main():
    ...
    performBackTracking(NewScheduleArray,check)
...
def performBackTracking(NewScheduleArray,check):
    global checkFlag
    flag = False
    for sel in NewScheduleArray:
    ...
    for ta in TAArray:
        checkFlag = []
        checkTAResponsibilitiesClashWithClass(unassignedClass,ta)
        checkTASkillSetCommon(unassignedClass,ta)
        checkTAResponsibilitiesClashWithCourseRecitation(unassignedClass,ta)
        for key in ta.contriList.keys():
            checkClassCollision(unassignedClass,key)
        if False not in checkFlag:
            assignTA(unassignedClass,ta)
            if unassignedClass.taRequiredNumber == 0:
                unassignedClass.closed = True
                keyword = performBackTracking(NewScheduleArray,check)
